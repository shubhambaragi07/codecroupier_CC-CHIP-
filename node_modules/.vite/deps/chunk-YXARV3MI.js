...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from19, paymaster, paymasterInput } = transaction;
  ...
  if (from19 && !isAddress(from19))
    throw new InvalidAddressError({ address: from19 });
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce: nonce2, to, from: from19, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce: nonce2, to, from: from19, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from19),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from19, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from19,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from19, to } = transaction;
  if (from19 && !isAddress(from19))
    throw new InvalidAddressError({ address: from19 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
// node_modules/viem/_esm/chains/definitions/battlechain.js
var battlechain = defineChain({
  ...chainConfig,
  id: 626,
...
// node_modules/viem/_esm/chains/definitions/defiOracleMetaMainnet.js
var defiOracleMetaMainnet = defineChain({
  id: 138,
...
// node_modules/viem/_esm/chains/definitions/marooTestnet.js
var marooTestnet = defineChain({
  id: 450815,
  name: "Maroo Testnet",
...
// node_modules/viem/_esm/chains/definitions/marsCredit.js
var marsCredit = defineChain({
  id: 110110,
  name: "Mars Credit",
  nativeCurrency: {
    decimals: 18,
...
// node_modules/viem/_esm/chains/definitions/robinhoodTestnet.js
...
__export(SignatureEnvelope_exports, {
  ...
  InvalidMultisigApprovalError: () => InvalidMultisigApprovalError,
...
// node_modules/viem/node_modules/ox/_esm/tempo/MultisigConfig.js
...
function getAddress(config) {
  assert4(config);
  const hash4 = keccak2562(concat(fromString(accountDomain), padLeft2(config.salt ?? zeroSalt, 32), fromNumber(config.threshold, { size: 1 }), fromNumber(config.owners.length, { size: 1 }), ...config.owners.flatMap((owner) => [
    owner.owner,
    fromNumber(owner.weight, { size: 1 })
  ])));
  const account = from2(slice(hash4, 12, 32));
  if (toBigInt(account) === 0n)
...
function getSignPayload(value) {
  ...
  return keccak2562(concat(fromString(signatureDomain), from(payload), account));
}
...
function assert5(envelope) {
  ...
  if (type2 === "multisig") {
    ...
    if (getNestingDepth(multisig) > maxNestingDepth)
      throw new InvalidMultisigApprovalError({
        reason: `multisig nesting depth exceeds ${maxNestingDepth}`
      });
...
function getNestingDepth(envelope) {
  let deepest = 0;
...
function deserialize(value) {
  ...
  if (typeId === serializedMultisigType) {
    const [account, signatures, init] = toHex2(data);
    if (typeof init !== "undefined" && !Array.isArray(init))
      throw new InvalidSerializedError({
        reason: "Invalid multisig `init`: expected the bootstrap config list or an omitted trailing element",
        serialized
      });
    return {
      type: "multisig",
...
function from7(value, options) {
  ...
  if (type2 === "multisig") {
    ...
    return {
      ...rest,
      account,
      signatures: rest.signatures.map((signature) => from7(signature)),
      ...
      ...initSource ? { init: from6(initSource) } : {},
      type: type2
    };
  }
  return {
...
function fromRpc2(envelope) {
  ...
  if (envelope.type === "multisig" || "account" in envelope && "signatures" in envelope) {
    const multisig = envelope;
    return {
      type: "multisig",
      ...
      ...multisig.init ? { init: from6(multisig.init) } : {}
    };
  }
...
function getType(envelope) {
  ...
  if ("userAddress" in envelope && "inner" in envelope)
    return "keychain";
  if (("account" in envelope || "genesisConfig" in envelope) && "signatures" in envelope)
    return "multisig";
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "multisig") {
    const multisig = envelope;
    return concat(serializedMultisigType, fromHex2([
      multisig.account,
      multisig.signatures.map((signature) => serialize(signature)),
      ...multisig.init ? [toTuple2(multisig.init)] : []
...
function sortMultisigApprovals(value) {
  ...
  const digest = getSignPayload("genesisConfig" in value && value.genesisConfig ? { payload, genesisConfig: value.genesisConfig } : { payload, account: value.account });
...
var InvalidMultisigApprovalError = class extends BaseError2 {
  constructor({ reason }) {
...
// node_modules/viem/node_modules/ox/_esm/tempo/Channel.js
var Channel_exports = {};
...
// node_modules/viem/node_modules/ox/_esm/tempo/Channel.js
var channelIdParameters = from3("address, address, address, address, bytes32, address, bytes32, address, uint256");
var domainSeparatorParameters = from3("bytes32, bytes32, bytes32, uint256, address");
var voucherHashParameters = from3("bytes32, bytes32, uint96");
...
function getVoucherSignPayload(value) {
  ...
  return keccak2562(concat("0x1901", domainSeparator({ chainId: value.chainId }), voucherHash));
}
...
function from12(value) {
  return keccak2562(concat(padLeft2(toAddress(value.userToken), 32), padLeft2(toAddress(value.validatorToken), 32)));
}
...
// node_modules/viem/node_modules/ox/_esm/tempo/ReceivePolicyReceipt.js
var transferBlocked = from13("event TransferBlocked(address indexed token, address indexed receiver, uint64 indexed blockedNonce, uint256 amount, uint8 receiptVersion, bytes receipt)");
...
function serialize4(envelope, options = {}) {
  ...
  return concat(options.format === "feePayer" ? feePayerMagic : serializedType, fromHex2(serialized));
}
...
function getSignPayload3(envelope, options = {}) {
  const sigHash = hash2(envelope, { presign: true });
  if (options.from)
...
function from15(value) {
  return from2(concat(toFixedHex(value.masterId, 4), magic, toFixedHex(value.userTag, 6)));
}
...
function serialize5(authentication, options = {}) {
  ...
  return concat(serialize(from7(signature)), getFields(authentication));
}
...
async function serializeTempo(transaction, sig) {
  ...
  const shouldStripFeeTokenForSponsorship = (
    ...
    feePayer === true && (!signature_provided || !feePayerSignature) || // Local fee-payer accounts sign after sender serialization.
    typeof feePayer === "object" && !signature_provided || // Filled transactions can already include fee-payer approval.
  ...
  const signature = (() => {
    ...
    if (!transaction.multisig || !transaction.signatures)
      return void 0;
    const payload = TxEnvelopeTempo_exports.getSignPayload(TxEnvelopeTempo_exports.from(transaction_sender_ox));
  ...
  if (signature && typeof transaction.feePayer === "object") {
    ...
    const sender = transaction.from ?? SignatureEnvelope_exports.extractAddress({
  ...
  if (feePayer === true || !signature && hasPrefilledFeePayerSignature) {
    if (signature && feePayerSignature)
      return TxEnvelopeTempo_exports.serialize(transaction_sender_ox, {
        signature
      });
    if (signature)
      return TxEnvelopeTempo_exports.serialize(transaction_sender_ox, {
...
async function signVoucher(account, parameters) {
  const hash4 = getVoucherSignPayload2(parameters);
  if (isAccessKeyAccount(account))
    return account.sign({ hash: hash4, raw: true });
  return account.sign({ hash: hash4 });
}
function getVoucherSignPayload2(parameters) {
...
async function signKeyAuthorization(account, parameters) {
  ...
  const hash4 = KeyAuthorization_exports.getSignPayload({
  ...
  const signature = isAccessKey ? await account.sign({ hash: hash4, raw: true }) : await account.sign({ hash: hash4 });
...
function resolveAccessKey(accessKey) {
  ...
  if ("publicKey" in accessKey && accessKey.publicKey)
    return {
      accessKeyAddress: fromPublicKey(fromHex(accessKey.publicKey)),
...
var tip20ChannelReserve2 = "0x4d50500000000000000000000000000000000000";
...
function resolveToken(client, parameters) {
  ...
  const address2 = TokenId_exports.toAddress(token);
  return {
    address: address2,
    decimals: decimals ?? (client ? inferDecimals(client, address2) : void 0)
  };
}
...
function pickWriteParameters(parameters) {
  const { account, chain, feePayer, feeToken, gas, keyAuthorization, maxFeePerGas, maxPriorityFeePerGas, nonce: nonce2, nonceKey, validAfter, validBefore } = parameters;
  return {
    account,
    chain,
    feePayer,
...
async function verifyHash2(client, parameters) {
  const { account, admin, hash: hash4, signature, ...rest } = parameters;
  return readContract(client, {
    ...rest,
    ...verifyHash2.call({ account, admin, hash: hash4, signature })
  });
}
(function(verifyHash3) {
  function call(args) {
    const { account, admin = true, hash: hash4, signature } = args;
    return defineCall({
      address: signatureVerifier2,
      abi: signatureVerifier,
      functionName: admin ? "verifyKeychainAdmin" : "verifyKeychain",
      args: [account, hash4, signature]
    });
  }
...
function formatTransactionRequest2(r, action) {
  ...
  const { multisig: _multisig, multisigInit, multisigSignatureCount, signatures: _signatures, ...rpcRequest } = request;
...
var chainConfig5 = {
  ...
  prepareTransactionRequest: [
    async (r, { client, phase }) => {
      ...
      if (multisig) {
        const config = MultisigConfig_exports.from(multisig);
        request.multisig = config;
        request.from = MultisigConfig_exports.getAddress(config);
        request.multisigInit = {
          salt: config.salt ?? MultisigConfig_exports.zeroSalt,
  ...
  async verifyHash(client, parameters) {
    const { address: address2, hash: hash4, signature, mode: mode2 } = parameters;
    const envelope = (() => {
    ...
    if (envelope) {
      if (envelope?.type === "keychain" && mode2 === "allowAccessKey") {
        const innerPayload = envelope.version === "v2" ? keccak256(concat("0x04", hash4, address2)) : hash4;
...
export {
  ...
  validatorConfig,
  signVoucher,
  accountKeychain2 as accountKeychain,
  accountRegistrar,
  addressRegistry2,
  feeManager2,
  nonceManager,
  pathUsd,
  receivePolicyGuard2,
  signatureVerifier2 as signatureVerifier,
  stablecoinDex2,
  tip20ChannelReserve2,
  tip20Factory2,
  tip403Registry2,
  validator,
  zoneOutbox,
...
//# sourceMappingURL=chunk-YXARV3MI.js.map
